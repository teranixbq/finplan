...
app.post('/', async (c) => {
  ...
  const existing = await db.select().from(months)
    .where(and(eq(months.month, body.month), eq(months.year, body.year))).get();
  ...
  const result = await db.insert(months).values({
    ...
    salary: body.salary ?? 0, salaryDate: body.salaryDate ?? 28,
    createdAt: now,
  }).returning().get();
  ...
  const prev = await db.select().from(months)
    .where(and(eq(months.month, prevMonth), eq(months.year, prevYear))).get();

  if (prev) {
    const prevExpenses = await db.select().from(expenses).where(eq(expenses.monthId, prev.id)).all();
    for (const e of prevExpenses) {
      await db.insert(expenses).values({
        monthId: result.id, assetId: e.assetId, name: e.name,
        category: e.category, amount: e.amount,
        periodMonths: e.periodMonths, periodType: e.periodType,
        isActive: e.isActive,
      });
    }

    const prevSummary = await getSummaryData(db, prev.id);
    ...
    for (const asset of allAssets) {
      ...
      const prevAssetIncomes = await db.select().from(incomes)
        .where(and(eq(incomes.monthId, prev.id), eq(incomes.assetId, asset.id))).all();
      const totalOut = prevAssetExpenses.reduce((s, e) => s + e.amount, 0);
      const totalIn = prevAssetIncomes.reduce((s, i) => s + i.amount, 0);
      const carryover = asset.amount + totalIn - totalOut;
      if (carryover !== asset.amount) {
        await db.update(assets).set({ amount: Math.max(0, carryover) }).where(eq(assets.id, asset.id));
      }
    }
  }
...
app.get('/:id/summary', async (c) => {
  ...
  const { allAssets, allInvestments, allExpenses, allIncomes, allDaily } = await getSummaryData(db, id);

  const totalCash = allAssets.reduce((s, a) => s + a.amount, 0);
  const totalInvestment = allInvestments.reduce((s, i) => s + i.amount, 0);
  const totalFixed = allExpenses.filter(e => e.isActive && e.category === 'fixed').reduce((s, e) => s + e.amount, 0);
  const totalVariable = allExpenses.filter(e => e.isActive && e.category === 'variable').reduce((s, e) => s + e.amount, 0);
  const totalPeriodic = allExpenses.filter(e => e.isActive && e.category === 'periodic').reduce((s, e) => s + e.amount, 0);
  ...
  const avgDaily = allDaily.length > 0
    ? totalDaily / Object.keys(dailyByDate).length
    : 0;
